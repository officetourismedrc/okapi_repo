    ...
    <div class="province_container">
        ...
        <div class="province__main">
            <div class="province__main-top">
                 <form action="{{route('decoupage-Administratif.create')}}" method="GET"  class="province__add">
                    <button>Add new Decoupage category</button>
                 </form>
            </div>
                    ...
                    <table class="province__table">
                        ...
                        <tbody>
                            ...
                            <tr class="table-body-container">
                               ...
                               <td class="table-body-cell"><a href="{{route('decoupage-Administratif.show',['decoupage_Administratif'=>$item->id])}}">{{$item->name}}</a></td>
                               <td class="table-body-cell">{{$item->description}}</td>
                               <td class="table-body-cell">
                                    <form action="{{route('decoupage-Administratif.edit',['decoupage_Administratif'=>$item->id])}}" method="get">
                                        @csrf
                                         <button>edit {{$item->id}}</button>
                                    </form>
                                    <form action="{{ route('decoupage-Administratif.destroy',['decoupage_Administratif'=>$item->id]) }}" method="post">
                                        @csrf
                                        @method('DELETE')
                                        <button>delete</button>
                                   </form>
                                   <form action="{{ route('decoupage-Administratif.show',['decoupage_Administratif'=>$item->id]) }}" method="get">
                                    @csrf
                                    <button>show</button>
                               </form>
                                </td>
